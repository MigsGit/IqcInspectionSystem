        ...
        $iqcInspectionCollection = collect($weekRanges)->map(function ($week)use($material_category,$model) {
            return $iqcInspectionPerSupplierCollection = $model::select(['supplier'])
                ->addSelect(
                    ...
                    DB::raw("COUNT(CASE WHEN judgement = 1 OR judgement = 3 THEN 1 END) as accepted_count"),
                    ...
                    DB::raw("ROUND( COUNT( CASE WHEN judgement = 1 OR judgement = 3 THEN 1 END ) / ( SUM(lot_inspected) ) * 100,2) as 'actual_lar' "), //lot accepted / lot inspected * 100 - ROUND OF 2
